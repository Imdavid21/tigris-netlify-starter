...
export function Holders({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    setError(false);
    setPage(1);
    fetch(API_URL + "/tokens/" + token + "/holders", {signal: AbortSignal.timeout(12000)})
      .then((r) => {
  ...
  return (
    <section className={ui("data-panel")}>
      ...
      {items.length > 10 && (
        <nav className={ui("pagination")} aria-label="Holder pages">
          <button disabled={page === 1} onClick={() => setPage((p) => p - 1)}>
            Previous
          </button>
          <span>
            {page} / {Math.ceil(items.length / 10)}
          </span>
          <button

...
export function SearchPalette() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(false);
    fetch(API_URL + "/tokens?limit=100", {signal: AbortSignal.timeout(12000)})
      .then((r) => {
  ...
  return (
    <>
      <button className={ui("search-trigger")} onClick={() => setOpen(true)}>
        <span>Search tokens</span>
        <kbd>⌘K</kbd>
      </button>
      <dialog
        ref={dialog}
        className={ui("search-modal")}
        aria-label="Search markets"
        onCancel={() => setOpen(false)}
        onClose={() => setOpen(false)}
      >
        <div className={ui("search-dialog-head")}>
          <input
            aria-label="Search markets"
            autoFocus
            value={query}
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && filtered[0])
                window.location.href = "/token/" + filtered[0].address;
            }}
          />
          <button
            type="button"
        ...
        <div className={ui("search-results")}>
          {loading || error || !filtered.length ? (
            ...
            filtered.map((x) => (
              <a
                ...
                onClick={() => setOpen(false)}
              >
                <span className={ui("search-avatar")}>
                  {x.symbol.slice(0, 2)}
                </span>
                <span>
          ...
          {/^(0x)[a-fA-F0-9]{40}$/.test(query.trim()) && (
            <a href={"/token/" + query.trim()}>Open this contract address ↗</a>
          )}
        </div>
        <div className={ui("search-help")}>

...
import { MarketCard, progress, type Launch, type IndexedLaunch } from "./market-card";
...
export function Launches() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) { setItems(indexed); setLoading(false); }
  ...
  const filtered = useMemo(() => {
    ...
    const matches = items.filter((x) => {
      ...
      const matchPair = pair === "all" || (x.quote_asset ?? "0x3600000000000000000000000000000000000000").toLowerCase() === pair;
      return matchQuery && matchPair && (!cutoff || time >= cutoff);
    });

    return [...matches].sort((a, b) => {
      if (sort === "volume" && pair !== "all") { const av = BigInt(a.volume_24h ?? "0"), bv = BigInt(b.volume_24h ?? "0"); return av === bv ? 0 : bv > av ? 1 : -1; }
  ...
  return (
    <div className={ui("explore-surface")}>
      <div className={ui("discovery-heading")}><div><h1>Find your next market.</h1><p>Discover, launch, and trade on Arc.</p></div><div className={ui("discovery-links")}><a className={ui("secondary-link")} href="/stocks">Stocks</a><a className={ui("primary-link")} href="/create">+ Create token</a></div></div>
      <div className={ui("discovery-summary")} aria-label="Loaded market summary">
        <div><span>Markets loaded</span><strong>{loading ? "…" : items.length}</strong></div>
        <div><span>On the curve</span><strong>{loading ? "…" : items.filter(x => x.status !== "GRADUATED").length}</strong></div>
        <div><span>Graduated</span><strong>{loading ? "…" : items.filter(x => x.status === "GRADUATED").length}</strong></div>
      </div>
      {error && (
        <div className={ui("system-notice")}>
          <strong>Market index degraded</strong>
          <span>{error} Open a known token address to check its onchain market.</span><button type="button" onClick={() => setRetry(x => x + 1)}>Retry</button>
        </div>
      )}
      ...
      <div className={ui("market-toolbar dense")}>
        ...
        <div className={ui("market-toolbar-actions")}>
          <select aria-label="Quote asset" value={pair} onChange={e => setPair(e.target.value)} className={ui("pair-filter")}><option value="all">All pairs</option><option value="0x3600000000000000000000000000000000000000">USDC</option><option value="0x89b50855aa3be2f677cd6303cec089b5f319d72a">EURC</option><option value="0xf0c4a4ce82a5746abaad9425360ab04fbba432bf">cirBTC</option></select>
          <div className={ui("segmented")}>
            {([
              ...
              <button key={value} disabled={value === "volume" && pair === "all"} title={value === "volume" && pair === "all" ? "Select a quote asset to compare volumes" : undefined} aria-pressed={sort === value} className={ui(sort === value ? "active" : "")} onClick={() => setSort(value)}>
                {label}
              </button>
            ))}
          </div>
          <div className={ui("segmented compact-segment")}>
            {(["all", "24h", "7d"] as const).map((value) => (
              <button key={value} aria-pressed={windowFilter === value} className={ui(windowFilter === value ? "active" : "")} onClick={() => setWindowFilter(value)}>
                {value === "all" ? "All" : value}
              </button>
            ))}
          </div>
      ...
      <section className={ui("market-section explore-grid-section")}>
        <div className={ui("market-section-head")}>
          ...
          <a href="/create" className={ui("secondary-link compact-link")}>+ Create</a>
        </div>

        {loading ? <div className={ui("market-card-grid")} aria-label="Loading markets" aria-busy="true">{Array.from({length:10}, (_,i) => <div className={ui("market-card")} key={i}><div className={ui("skeleton skeleton-image")} /><div className={ui("skeleton skeleton-line")} /><div className={ui("skeleton skeleton-line")} /></div>)}</div> : !live.length ? (
          ...
          <div className={ui("market-card-grid")}>
            {live.slice((page - 1) * 20, page * 20).map((item) => <MarketCard item={item} key={item.address} />)}
          </div>
        )}
        {pageCount > 1 && <nav className={ui("pagination")} aria-label="Market pages"><button disabled={page === 1} onClick={() => setPage(p => p - 1)}>Previous</button><span>{page} / {pageCount}</span><button disabled={page === pageCount} onClick={() => setPage(p => p + 1)}>Next</button></nav>}
      </section>
    </div>
  );
}

...
export function ProfilePanel() {
  ...
  const [tab,setTab]=useState<"launches"|"positions"|"orders"|"activity">("positions");
  ...
  if(!address){
    return <div className={ui("profile-connect")}>
      <div className={ui("profile-connect-mark")}><CelestialLogo /></div>
  ...
  return <div className={ui("profile-layout")}>
    <section className={ui("profile-summary")}>
      <div><span className={ui("kicker")}>Wallet</span><strong>{address.slice(0,7)}...{address.slice(-5)}</strong></div>
      <div className={ui("profile-summary-stat")}><span>Launches</span><strong>{launches.length}</strong></div>
      <div className={ui("profile-summary-stat")}><span>Positions</span><strong>{positions.length}</strong></div>
      <div className={ui("wallet-tools")}><a href={"https://testnet.arcscan.app/address/"+address} target="_blank" rel="noreferrer">Explorer ↗</a><button onClick={()=>void refresh(address)}>Refresh</button></div>
    </section>

    <section className={ui("fees-card")}>
      <div>
        <span className={ui("kicker")}>Creator fees</span>
    ...
    {celestialAddresses.feeEscrow && (
      <section className={ui("metric-grid")}>
        {quoteAssets.map((asset)=>(
          <div className={ui("metric-card")} key={asset.symbol}>
    ...
    <div className={ui("profile-tabs")}>
      <button className={ui(tab==="launches"?"active":"")} onClick={()=>setTab("launches")}>My launches</button>
      <button className={ui(tab==="positions"?"active":"")} onClick={()=>setTab("positions")}>Positions</button>
      <button className={ui(tab==="orders"?"active":"")} onClick={()=>setTab("orders")}>Orders</button>
      <button className={ui(tab==="activity"?"active":"")} onClick={()=>setTab("activity")}>Activity</button>
    </div>

    <section className={ui("profile-table")}>
      {tab==="launches" && (launches.length?launches.map(x=>
        <a href={"/token/"+x.address} className={ui("profile-row")} key={x.address}>
      ...
      ):<div className={ui("profile-empty")}>No launches from this wallet yet.</div>)}

      {tab==="positions" && (positions.length?positions.map(x=>
        <a href={"/token/"+x.token} className={ui("profile-row")} key={x.token}>
          <div><strong>{x.name}</strong><span>{"$"+x.symbol}</span></div>
          <span>{Number(formatUnits(BigInt(x.balance),18)).toLocaleString(undefined,{maximumFractionDigits:0})}</span>
          <span>{x.status==="GRADUATED"?"Graduated":"Curve"}</span>
        </a>
      ):<div className={ui("profile-empty")}>No indexed token positions yet.</div>)}

      {tab==="orders" && (orders.length?orders.map(x=>
        <div className={ui("profile-row")} key={x.order_id}>
      ...
      ):<div className={ui("profile-empty")}>No indexed limit orders yet.</div>)}

      {tab==="activity" && (activity.length?activity.map(x=>
        <a href={"/token/"+x.token} className={ui("profile-row")} key={x.tx_hash}>
      ...
      ):<div className={ui("profile-empty")}>No indexed activity yet.</div>)}
    </section>
